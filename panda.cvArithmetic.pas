...
function Max_NU8(pA: PUInt8; N: NativeInt): UInt8;

procedure VecMin(pA, pB, pRes: PUInt8; aCount: NativeInt); overload;
procedure VecMin(pA, pB, pRes: PDouble; aCount: NativeInt); overload;

procedure VecMax(pA, pB, pRes: PUInt8; aCount: NativeInt); overload;
...
// pRes <- (pX - pY)^2
procedure SQDiffs(pX, pY, pRes: PDouble; aCount: NativeInt); overload;
procedure SQDiffs(pX, pY, pRes: PSingle; aCount: NativeInt); overload;
...
const cSgnMask: UInt64 = $8000000080000000;
{$if defined(ASMx86)}
// EAX <- pA, EDX <- pRes, ECX <- aCount
asm
{$ifdef DEBUG}
...
begin
  ...
  pEndu := PByte(pA) + (aCount and (not NativeInt(3))) * SizeOf(Single);
  while PByte(pA) < pEndu do begin
    PUInt64(pRes)^ := PUInt64(pA)^ xor cSgnMask;
    Inc(pRes, 2);
    Inc(pA, 2);
    PUInt64(pRes)^ := PUInt64(pA)^ xor cSgnMask;
    Inc(pRes, 2);
...
procedure VecAbs(pA, pRes: PDouble; aCount: NativeInt);
var pEnd: PByte;
begin
  pEnd := PByte(pA) + aCount * SizeOf(Double);
  while PByte(pA) < pEnd do begin
...
procedure VecAbs(pA, pRes: PSingle; aCount: NativeInt);
const cMask = $7FFFFFFF7FFFFFFF;
var pEnd, pEndu: PByte;
begin
  pEnd := PByte(pA) + aCount * SizeOf(Single);
  pEndu := PByte(pA) + (aCount and (not NativeInt(3))) * SizeOf(Single);
  while PByte(pA) < pEndu do begin
    PUInt64(pRes)^ := PUInt64(pA)^ and cMask;
    Inc(pRes, 2);
    Inc(pA, 2);
    PUInt64(pRes)^ := PUInt64(pA)^ and cMask;
    Inc(pRes, 2);
...
{$ifdef AVX}
  ...
  // duplicate real parts of A
  vpermilpd ymm2, ymm0, 0  // ar0,ar0, ar1,ar1
  // duplicate imag parts of A
  vpermilpd ymm3, ymm0, 15 // ai0,ai0, ai1,ai1
  // real contribution
  vmulpd ymm2, ymm2, ymm1  // ar*br , ar*bi
  // swap real/imag in B
  vpermilpd ymm4, ymm1, 5  // bi0,br0, bi1,br1
  // imag contribution
  vmulpd ymm3, ymm3, ymm4  // ai*bi , ai*br
  // combine
  vaddsubpd ymm0, ymm2, ymm3
  vmovupd [r8], ymm0
  add rcx, 32
  add rdx, 32
  add r8, 32
...
procedure SQDiffs(pX, pY, pRes: PDouble; aCount: NativeInt);
{$if defined(ASMx64)}
{$CODEALIGN 16}
...
procedure SQDiffs(pX, pY, pRes: PSingle; aCount: NativeInt);
{$if defined(ASMx64)}
// RCX <- pX, RDX <- pY, R8 <- pRes, R9 <- aCount
asm
  mov r10, r9
...
@init:
  mov     r8, rcx             // r8 := ptr
  xorps xmm0, xmm0          // xmm0 := 0x00 in all lanes (start min for unsigned)
...
@L:
  ...
  movdqu  xmm1, [r8]          // load 16 bytes (unaligned ok)
  pmaxub  xmm0, xmm1          // running per-byte max
  add     r8, 16
  sub     rdx, 16
  jmp     @L
...
procedure VecMin(pA, pB, pRes: PUInt8; aCount: NativeInt);
{$if defined(ASMx64)}
// RCX <- pA, RDX <- pB, R8 <- pRes, R9 <- aCount
asm
  mov r10, r9
...
procedure VecMin(pA, pB, pRes: PDouble; aCount: NativeInt);
{$if defined(ASMx64)}
// RCX <- pA, RDX <- pB, R8 <- pRes, r9 <- aCount
asm
  mov r10, r9
...
procedure VecMax(pA, pB, pRes: PUInt8; aCount: NativeInt);
{$if defined(ASMx64)}
// RCX <- pA, RDX <- pB, R8 <- pRes, R9 <- aCount
asm
  mov r10, r9
...
procedure VecMax(pA, pB, pRes: PDouble; aCount: NativeInt);
{$if defined(ASMx64)}
// RCX <- pA, RDX <- pB, R8 <- pRes, r9 <- aCount
asm
  mov r10, r9
...
procedure VecMax(A: Single; pB, pRes: PSingle; aCount: NativeInt);
{$if defined(ASMx64)}
// XMM0 <- A, RDX <- pB, R8 <- pRes, R9 <- aCount
asm
  mov r10, r9
